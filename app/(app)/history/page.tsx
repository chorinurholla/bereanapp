...
export default function FormationRecordPage() {
  ...
  // Format a devotion reply for full readable display
  function formatDevotionReply(text: string): string {
    if (!text) return ''
    const ps = 'border-left:4px solid var(--gold2);background:rgba(138,109,53,0.07);padding:14px 18px;margin:14px 0;font-family:Playfair Display,serif;font-size:17px;font-style:italic;color:var(--gold);line-height:1.6'
    const hs = 'font-family:DM Sans,sans-serif;font-size:9.5px;font-weight:600;letter-spacing:0.2em;text-transform:uppercase;color:var(--gold);margin:20px 0 8px;padding-bottom:6px;border-bottom:1px solid var(--rule)'
    let t = text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
    t = t.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>')
    t = t.replace(/^#{1,3} (.+)$/gm, '<h3 style="' + hs + '">$1</h3>')
    t = t.replace(/\[PRINCIPLE\]([\s\S]*?)\[\/PRINCIPLE\]/g, '<div style="' + ps + '">$1</div>')
    t = t.replace(/\[WARNING\]([\s\S]*?)\[\/WARNING\]/g, '<div style="border-left:3px solid rgba(122,42,42,0.4);padding:8px 14px;margin:10px 0;font-size:13px;color:rgba(122,42,42,0.8)">⚠ $1</div>')
    t = t.split('\n\n').map(p => p.trim() ? '<p style="margin-bottom:14px;font-family:Source Serif 4,serif;font-size:16px;line-height:1.85;color:var(--ink2)">' + p.replace(/\n/g,'<br>') + '</p>' : '').join('')
    return t
  }
  return (
          ...
          <div>
            {history.length === 0 ? (
              ...
              history.map(h => {
                ...
                // Format the reply for full readable display
                const formattedReply = formatDevotionReply(h.reply)
                ...
                return (
                  <div key={h.id}
                    ...
                    <div
                      ...
                      style={{
                        padding: open ? '18px 24px 14px 21px' : '18px 24px',
                        cursor: 'pointer',
                        background: open ? 'var(--paper2)' : 'transparent',
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        color: 'var(--ink5)', fontSize: '16px', marginTop: '2px',
                    ...
                    {open && (
                      <div className="fade-in" style={{
                        ...
                        <div
                          className="berean-prose"
                          style={{ fontSize:'16px', lineHeight:1.85, paddingTop:'20px' }}
