...
export function useCorpusChat({
  ...
  const buildSystemPrompt = useCallback((chapters: CorpusChapter[], query: string, detectedPattern: ReturnType<typeof detectPattern>['pattern'], devLength: 'brief' | 'standard' | 'deep' = 'standard') => {
    const name = profile?.name || 'the reader'
    const occ  = profile?.occupation || 'daily life'
    const ctx  = buildContextBlock(chapters)

    if (devoMode) {
      // Length-specific format instructions
      const lengthConfig = {
        brief: {
          label: 'BRIEF (400-600 words)',
          format: [
            '**OBSERVATION** — 1-2 sentences of narrative context.',
            '**TIMELESS PRINCIPLE**',
            '[PRINCIPLE]One clear memorable sentence.[/PRINCIPLE]',
            '**APPLICATION** — 2-3 sentences specific to ' + occ + '.',
            '**PRAYER** — A focused 4-6 line prayer covering Identity, Promise, Alignment, Declaration.',
            'Close: "In Jesus\' Name, Amen."',
            '',
            'Total: 400-600 words. Concise but spiritually nourishing.',
          ].join('\n'),
          maxTokens: 800,
        },
        standard: {
          label: 'STANDARD (900-1200 words)',
          format: [
            '**OBSERVATION** — 2-3 sentences.',
            '**INTERPRETATION** — What it means. Flag any interpretive leap.',
            '**TIMELESS PRINCIPLE**',
            '[PRINCIPLE]One sentence — memorable, transferable.[/PRINCIPLE]',
            '**MODERN APPLICATION** — 3-4 sentences specific to ' + occ + '.',
            '**CROSS-REFERENCES** — 2 brief cross-references.',
            '**PRAYER** — Four-part prayer: Identity, Covenant Promises, Alignment, 2-3 Declarations.',
            'Close: "In Jesus\' Name, Amen."',
            '',
        ...
        deep: {
          label: 'DEEP STUDY (1400-1800 words)',
          format: [
            '**OBSERVATION** — Full narrative context, covenant setting, key details. 3-4 sentences.',
            '**INTERPRETATION** — Detailed exegesis. Flag interpretive leaps explicitly.',
            '**TIMELESS PRINCIPLE**',
            '[PRINCIPLE]One sentence — memorable, transferable, rooted in the text.[/PRINCIPLE]',
            '**CRITICAL ASSESSMENT**',
            '- Support strength: Strong / Moderate / Weak',
            '- Interpretive leap: flag if present',
            '- Alternative interpretation',
            '**MODERN APPLICATION** — 4-5 sentences highly specific to ' + occ + '.',
            '**CROSS-REFERENCES** — 3 references (same book, OT parallel, NT echo). Similarity AND difference.',
            '**GENRE & COVENANT CONTEXT** — Genre note. Misapplication warning.',
            '**PRAYER** — Full four-part prayer: Identity in Christ, Covenant Promises, Alignment with God\'s Will, 3 bold Declarations.',
            'Close: "In Jesus\' Name, Amen."',
            '',
            'Total: 1400-1800 words. Theologically rigorous AND devotionally warm.',
          ].join('\n'),
          maxTokens: 2800,
        },
      }

      const cfg = lengthConfig[devLength]
  ...
  const sendMessage = useCallback(async (query: string) => {
    ...
    try {
      const system = buildSystemPrompt(chapters, query, topPattern, devLength)
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: 'claude-sonnet-4-5',
          max_tokens: devoMode ? (devLength === 'brief' ? 900 : devLength === 'deep' ? 3000 : 1800) : 1200,
          system,
